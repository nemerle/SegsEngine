...
//
    ...
    EASTL_API int Vsnprintf8(char* pDestination, size_t n, const char* pFormat, va_list arguments);
    EASTL_API int Vsnprintf16(char16_t* pDestination, size_t n, const char16_t* pFormat, va_list arguments);
    EASTL_API int Vsnprintf32(char32_t* pDestination, size_t n, const char32_t* pFormat, va_list arguments);
    #if EA_CHAR8_UNIQUE
        EASTL_API int Vsnprintf8 (char8_t*  pDestination, size_t n, const char8_t*  pFormat, va_list arguments);
    #endif
    #if defined(EA_WCHAR_UNIQUE) && EA_WCHAR_UNIQUE
    EASTL_API int VsnprintfW(wchar_t* pDestination, size_t n, const wchar_t* pFormat, va_list arguments);
    #endif
...
namespace eastl
{
    ...
    public:
       ...
       ~basic_string();
        ...
        this_type&  replace(const_iterator first, const_iterator last, view_type n) {
            return replace(first,last,n.data(),n.size());
        }
...
#if EASTL_LF3D_EXTENSIONS
        this_type& operator+=(view_type x) {
            return append(x);
        }
        ...
        void replace(view_type needle, view_type replacement)
        {
            if (needle.empty() || replacement.empty() )
                return;

            const size_type needle_length = needle.length();
            const size_type replacement_length = replacement.length();
            size_type position = 0;
            for (auto pos = find(needle, position); pos != npos; pos = find(needle, position))
            {
                replace(begin() + pos, begin() + pos + needle_length, replacement);
        ...
        this_type replaced(view_type needle, view_type replacement) const
        {
            this_type result(*this);
            result.replace(needle, replacement);
            return result;
        ...
        template<typename CONTAINER>
        static void split_ref(CONTAINER &tgt,const view_type str, const view_type separator, bool keepEmptyStrings = false)
        {
            const char* strEnd = str.data() + str.size();
            const char *start = str.data();
...
#if EASTL_LF3D_EXTENSIONS
//    template <typename T, typename Allocator>
//    basic_string<T, Allocator> operator+(typename basic_string<T, Allocator>::view_type v, const basic_string<T, Allocator>& b)
//    {
//        typedef typename basic_string<T, Allocator>::CtorDoNotInitialize CtorDoNotInitialize;
//        CtorDoNotInitialize cDNI; // GCC 2.x forces us to declare a named temporary like this.
//        basic_string<T, Allocator> result(cDNI, v.size() + b.size(), const_cast<basic_string<T, Allocator>&>(b).get_allocator());
//        result.append(v);
//        result.append(b);
//        return result;
//    }
    template <typename T, typename Allocator>
    basic_string<T, Allocator> operator+(const basic_string<T, Allocator>& a,typename basic_string<T, Allocator>::view_type v)
    {
        typedef typename basic_string<T, Allocator>::CtorDoNotInitialize CtorDoNotInitialize;
        CtorDoNotInitialize cDNI; // GCC 2.x forces us to declare a named temporary like this.
        basic_string<T, Allocator> result(cDNI, v.size() + a.size(), const_cast<basic_string<T, Allocator>&>(a).get_allocator());
        result.append(a);
        result.append(v);
        return result;
    }
    ...
    template <typename T, typename Allocator>
    inline bool operator<(typename basic_string<T, Allocator>::view_type a,const basic_string<T, Allocator>& b)
    {
        return basic_string<T, Allocator>::compare(a.begin(), a.end(), b.begin(), b.end()) < 0;
    }
